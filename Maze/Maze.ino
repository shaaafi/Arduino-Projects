...
int replaystage; int rightNudge; char path[30] = {}; int pathLength; int readLength; 
void setup(){
...
digitalWrite(rightMotor1, HIGH); }
digitalWrite(rightMotor2, LOW); if(path[pathLength-3]=='L' && path[pathLengthdelay(2);
1]=='S' && shortDone==0){
...
1]=='S' && shortDone==0){ digitalWrite(rightMotor1, HIGH);
...
void replay(){ delay(200);
readSensors(); digitalWrite(led, HIGH);
if(leftFarReading<200 && rightFarReading<200){ delay(500);
straight(); endMotion();
} }
else{
